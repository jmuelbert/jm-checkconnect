...
@click.option("-c", "--config", type=click.Path(exists=True), help="Path to the configuration file", default="config.ini")
...
@click.option("--version", is_flag=True, help="Show the version and exit.")  # Added version option
def main(gui: bool, config: str, output: str, verbose: bool, version: bool):
    ...
    console_handler_level_str = config_parser.get("Logging", "console_handler_level", fallback="DEBUG").upper()
    console_handler_level = getattr(logging, console_handler_level_str, logging.DEBUG)
    file_handler_level_str = config_parser.get("Logging", "file_handler_level", fallback="INFO").upper()
    file_handler_level = getattr(logging, file_handler_level_str, logging.INFO)

    file_handler_file = config_parser.get("Logging", "file_handler_file", fallback="checkconnect.log")  # Get log file name

    # Get formatter configuration
    formatter_format = config_parser.get("Logging", "simple_formatter_format", fallback="%(asctime)s - %(name)s - %(levelname)s - %(message)s")
    formatter_datefmt = config_parser.get("Logging", "simple_formatter_datefmt", fallback="")

    # Configure logging
    logging.basicConfig(level=log_level,
                        format=formatter_format,
                        datefmt=formatter_datefmt,
                        filename=file_handler_file)  # set filename
    ...
    if gui:
        ...
        translation = load_translation(translator, app, logger)  # Load and apply translation
