...
class TestCliGUI:
    ...
    def test_gui_command_from_main_runs_successfully(
        ...
        assert any(
            e.get("event") == "Main callback: is starting!" and e.get("log_level") == "debug"
            for e in caplog_structlog
        )
        assert any(
            e.get("event") == "CLI Args" and e.get("log_level") == "debug" and
            e.get("verbose") == 0 and e.get("language") is None and e.get("config_file") is None
            for e in caplog_structlog
        )

        # 2. Assert key INFO level success messages
        assert any(
            e.get("event") == "Main callback: SettingsManager initialized and configuration loaded." and e.get("log_level") == "info"
            for e in caplog_structlog
        )
        assert any(
        ...
        assert any(
            e.get("event") == "Main callback: Full logging configured based on application settings and CLI options." and e.get("log_level") == "info"
            for e in caplog_structlog
        )

        # 3. Assert GUI specific startup INFO log
        assert any(
            e.get("event") == "Starting CheckConnect GUI..." and e.get("log_level") == "info"
            for e in caplog_structlog
        )

        # 4. Assert CLI-Verbose and Logging Level determination (DEBUG)
        assert any(
            e.get("event") == "Main callback: Determined CLI-Verbose and Logging Level to pass to LoggingManager." and e.get("log_level") == "debug" and
            e.get("verbose_input") == 0 and e.get("derived_cli_log_level") == "WARNING"
            for e in caplog_structlog
        )
        ...
        assert not any(
            e.get("log_level") == "error" or e.get("log_level") == "critical"
            for e in caplog_structlog
    ...
    def test_gui_command_from_main_runs_with_languages(
        ...
        assert any(
            e.get("event") == "CLI Args" and e.get("log_level") == "debug" and
            e.get("verbose") == 0 and e.get("language") is language_value and e.get("config_file") is None
            for e in caplog_structlog
        )
    ...
    def test_gui_command_handles_exit_exception_from_subcommand(
        ...
        mocker.patch("checkconnect.cli.gui_app.startup.run",
            side_effect=ExitExceptionError("GUI failure"))
        ...
        assert any(
            e.get("event") == "Cannot start GUI due to application error: GUI failure" and
    ...
    def test_gui_command_handles_unexpected_exception(
        ...
        # Optional: Assert no ERROR/CRITICAL logs in a successful run
        assert any(
            e.get("event") == "An unexpected error occurred during GUI startup: Crash" and
            e.get("log_level") in ["error", "critical"]
            for e in caplog_structlog
        )
